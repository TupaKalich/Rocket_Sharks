...
class Price(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("country_id", "city_id", "court_id", "time_slot_id", "event_type", "day_of_week", name="uq_price_scope"),
    )
...
class CryptoAccount(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    exchange = db.Column(db.String(20), nullable=False, unique=True)
...
def serialize_model_state(model, fields):
    payload = {}
    for field in fields:
...
def seed_defaults():
    ...
    if not PaymentMethod.query.first():
        ...
        db.session.add_all(
            [
                CryptoAccount(
                    exchange="Bybit",
                    account_id="BYBIT_123456",
                    instruction="Внутренний перевод без комиссии",
                    comment="USDT TRC20",
                ),
                CryptoAccount(
                    exchange="HTX",
                    account_id="HTX_987654",
                    instruction="Внутренний перевод без комиссии",
                    comment="USDT TRC20",
                ),
            ]
    ...
    env_admin_email = __import__("os").getenv("BOOTSTRAP_ADMIN_EMAIL")
    env_admin_password = __import__("os").getenv("BOOTSTRAP_ADMIN_PASSWORD")

    if env_admin_email and env_admin_password and not User.query.filter_by(email=env_admin_email).first():
        admin_role = Role.query.filter_by(code="admin").first()
        admin = User(email=env_admin_email, role_id=admin_role.id, can_edit_requisites=True)
